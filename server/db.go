...
func readServer(guildID string) *Server {
	...
	var server Server
	err := db.Read(SERVER_COLLECTION, filter, &server)
	if err != nil {
		log.WithFields(log.Fields{"guild": guildID}).Info("server not found in the database")
		return nil
	}
	log.WithFields(log.Fields{"guild": server.GuildID}).Info("read server from the database")
	return &server
...
func writeServer(server *Server) error {
	...
	err := db.Write(SERVER_COLLECTION, filter, server)
	if err != nil {
